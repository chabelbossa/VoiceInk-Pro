...
enum ShortcutMigration {
    ...
    static func migrateLegacyKeyboardShortcutsIfNeeded() {
        ...
        migrateDefaultToggleEnhancementShortcutIfNeeded()
    ...
    private static func migrateDefaultToggleEnhancementShortcutIfNeeded() {
        guard
            ShortcutStore.rawShortcut(for: .toggleEnhancement) == nil,
            !ShortcutStore.isShortcutCleared(for: .toggleEnhancement)
        else {
            return
        }

        ShortcutStore.setShortcut(
            .key(keyCode: UInt16(kVK_ANSI_E), modifierFlags: [.command]),
