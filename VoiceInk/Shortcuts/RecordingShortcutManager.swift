...
class RecordingShortcutManager: ObservableObject {
    ...
    private func handleGlobalShortcut(_ action: ShortcutAction) async {
        ...
        case .toggleEnhancement:
            guard let currentMode = ModeManager.shared.currentEffectiveConfiguration else { return }
            let isEnabled = !currentMode.isAIEnhancementEnabled
            ModeManager.shared.updateCurrentEffectiveConfiguration { configuration in
                configuration.isAIEnhancementEnabled = isEnabled
            }
            NotificationManager.shared.showNotification(
                title: isEnabled ? "AI Enhancement enabled" : "AI Enhancement disabled",
